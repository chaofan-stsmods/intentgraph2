...
public class DevConsoleConstructorPatch
{
    public static void Postfix(MegaCrit.Sts2.Core.DevConsole.DevConsole __instance)
    {
        ...
        new Traverse(__instance).Method("RegisterCommand", [typeof(AbstractConsoleCmd)], null).GetValue(new GenerateMermaidConsoleCmd());
        IgLogger.Info("Registered generatemermaid to DevConsole.");
    }
}
